...
export default function CampaignsPage() {
  ...
  useEffect(() => {
    filterContacts()
  }, [form.network, allContacts, contactSearch])
  ...
  function filterContacts() {
    ...
    if (form.network !== 'All Contacts') {
      result = result.filter(c => c.tags?.includes(form.network))
    }
    if (contactSearch) {
          ...
          <form onSubmit={handleSend} className="space-y-5">
            ...
            <div className="flex gap-3 pt-2">
              <button type="submit" disabled={saving || selectedIds.size === 0}
                className="bg-blue-600 hover:bg-blue-700 disabled:bg-gray-300 disabled:text-gray-500 text-white px-6 py-2 rounded-lg text-sm font-medium flex items-center gap-2">
                {saving ? '⏳ Sending...' : "📣 Send to " + selectedIds.size + " contacts"}
              </button>
              <button type="button"
                onClick={() => { setShowForm(false); setSelectedIds(new Set()); removeFile() }}
              ...
              {broadcasts.map(b => (
                <tr key={b.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3">
                    <span className={"text-xs px-2 py-1 rounded-full font-medium " + (tagColors[b.audience] || 'bg-gray-100 text-gray-600')}>
                      {b.audience}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-sm text-gray-500 max-w-xs truncate">{b.message}</td>
                  <td className="px-4 py-3 text-sm text-gray-500">
                    {b.mediaType === 'image' ? '🖼️ Image' : b.mediaType === 'video' ? '🎥 Video' : b.mediaType === 'pdf' ? '📄 PDF' : '—'}
                  </td>
                  <td className="px-4 py-3 text-sm text-gray-900 font-medium">{b.recipientCount || 0} contacts</td>
                  <td className="px-4 py-3">
                    <span className="text-xs px-2 py-1 rounded-full font-medium bg-green-50 text-green-700">{b.status}</span>
                  </td>
                  <td className="px-4 py-3 text-sm text-gray-500">
